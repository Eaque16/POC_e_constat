...
def test_poc_call_starts_with_spoken_question_and_microphone():
    ...
    assert "nom de famille" in result[5].lower()
...
def test_claim_panel_exposes_captured_and_missing_states():
    ...
    state["data"] = {"nom_assure": "Jean Kouassi", "lastname": "Kouassi"}
...
def test_question_and_activity_are_explicit():
    ...
    assert "nom de famille" in current_question_text(state).lower()
...
def test_written_correction_replaces_a_bad_transcription(monkeypatch):
    state = new_conversation()
    monkeypatch.setattr(
        "econstat.ui.app.persist_conversation", lambda conversation: (conversation, None)
    )

    result = process_poc_text("Kouassi", [], state)

    assert result[2] == "Kouassi"
    assert result[3][0][0] == "✏️ Kouassi"
    assert result[4]["pending_confirmation"]["normalized"] == "KOUASSI"
    assert result[4]["current_field"] == "confirmation"
    assert result[9] == ""
